...
class AutoLoader(Vehicle):
    ...
    def __str__(self):
        res = super().__str__()
        return str(res + 'грузим {}'.format(self.truck))
...
while piter.content < TOTAL_CARGO:
    hour += 1
    print(Fore.RED, '------------------ Час {} ------------------'.format(hour), Style.RESET_ALL, sep='')
    for truck in trucks:
        truck.act()
    loader_1.act()
    loader_2.act()
...
print('Всего затрачено топлива', Vehicle.total_fuel)
print('Время простоя грузовиков', Truck.dead_time)
print('Время простоя погрузчиков', AutoLoader.dead_time)
